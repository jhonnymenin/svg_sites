...
/**
 * Evening in Jardins: the page's one night band. Distances are set like a
 * timetable — the minutes are the headline, the place is the caption.
 */
export function Location() {
  return (
    <section id="location" aria-labelledby="location-title" className="on-dark bg-night text-cream">
      <div className="frame grid gap-y-14 py-[clamp(80px,10vw,160px)] lg:grid-cols-12 lg:gap-x-[clamp(24px,3vw,56px)]">
        <div className="lg:col-span-4">
          <SectionHead id="location-title" eyebrow={l.eyebrow} title={l.title} intro={l.intro} tone="dark" />
        </div>

        <ol className="grid grid-cols-2 border-t border-cream/15 lg:col-span-8 lg:grid-cols-3">
          {l.places.map((p, i) => {
